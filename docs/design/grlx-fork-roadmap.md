...
| | |
|---|---|
| **Change** | Envoy in front of NATS's websocket listener, validating each sprout's JWT before the connection reaches nats-server — plus a second Envoy route proxying authenticated recipe-download requests through to non-DMZ. A new, separate enrollment endpoint (not JWT-gated, since a sprout has none yet) accepting a short-lived, tenant-scoped, usage-capped one-time registration key, backed by a new PXC table, returning the sprout's JWT + NKey identity + tenant X25519 public key in one response. |
