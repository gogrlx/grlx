...
// ListUsers retrieves all configured users grouped by role.
func ListUsers() (handlers.UsersByRole, error) {
	var users handlers.UsersByRole
	newToken, err := auth.NewToken()
	if err != nil {
		return users, err
	}
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return users, fmt.Errorf("list users: HTTP %d: %s", resp.StatusCode, string(body))
	}
